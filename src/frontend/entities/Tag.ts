...
import { ROOT_LOCATIONS_TAG_ID, ROOT_TAG_ID, TagDTO } from '../../api/tag';
...
export class ClientTag {
  ...
  // Indicates if the tag is a autogenerated location Tag and should not be reialized and saved into the files
  ...
  @action getSubTree(): Generator<ClientTag> {
    function* tree(
      ...
      if (path.has(tag) && tag.id !== ROOT_TAG_ID && tag.id !== ROOT_LOCATIONS_TAG_ID) {
  ...
  @action getAncestors(visited?: Set<ClientTag>): Generator<ClientTag> {
    function* ancestors(
      ...
      } else if (!visited.has(tag) && tag.id !== ROOT_TAG_ID && tag.id !== ROOT_LOCATIONS_TAG_ID) {
        path.add(tag);
        visited.add(tag);
        yield tag;
  ...
  @action.bound insertSubTag(tag: ClientTag, at: number): boolean {
    ...
    if (this === tag || tag.id === ROOT_TAG_ID || tag.id === ROOT_LOCATIONS_TAG_ID) {
