...
import { fsPromises } from 'common/platform/fsPromises';
...
export class ClientLocation {
  ...
  @action async init(): Promise<FileStats[] | undefined> {
    ...
    if (await fsPromises.pathExists(this.path)) {
    ...
    worker.onmessage = ({
      ...
      if (data.type === 'add') {
        ...
        setTimeout(async () => {
          ...
          const updatesStats = await fsPromises.stat(data.value.absolutePath);
...
async function getDirectoryTree(path: string): Promise<IDirectoryTreeItem[]> {
  try {
    ...
    const dirs = await Promise.all(
      Array.from(await fsPromises.readdir(path), async (file) => {
